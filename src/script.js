...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (staffTrack && staffPrev && staffNext) {

    const scrollAmount = 260;

    staffNext.addEventListener('click', () => {
      staffTrack.scrollBy({ left: scrollAmount, behavior: 'smooth' });
    });
    ...
    // Drag to scroll pakai mouse (PC)
    let isDown = false;
    let startX, scrollLeft;

    staffTrack.addEventListener('mousedown', (e) => {
      isDown = true;
      staffTrack.classList.add('dragging');
      startX = e.pageX - staffTrack.offsetLeft;
      scrollLeft = staffTrack.scrollLeft;
    });

    staffTrack.addEventListener('mouseleave', () => {
      isDown = false;
      staffTrack.classList.remove('dragging');
    });

    staffTrack.addEventListener('mouseup', () => {
      isDown = false;
      staffTrack.classList.remove('dragging');
    });
    ...
    staffTrack.addEventListener('wheel', (e) => {
      if (e.deltaY !== 0) {
        e.preventDefault();
        staffTrack.scrollLeft += e.deltaY;
      }
    }, { passive: false });
  }
